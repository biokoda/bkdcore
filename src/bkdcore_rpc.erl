...
connect_offset(Port) ->
	Off = application:get_env(bkdcore,connect_offset,0),
	Off + Port.
	...
	case distreg:reg({bkdcore,Node}) of
		ok ->
			case bkdcore:node_address(Node) of
				{IP,Port} ->
					case gen_tcp:connect(IP,connect_offset(Port),[{packet,4},{keepalive,true},binary,{active,once},
...
connect_to(Home,Node) ->
	case (catch bkdcore:node_address(Node)) of
		{IP,Port} when is_list(IP), is_integer(Port) ->
			case gen_tcp:connect(IP,connect_offset(Port),[{packet,4},{keepalive,true},binary,{active,false},
					{send_timeout,2000},{nodelay,true}],2000) of
				{ok,S} ->
